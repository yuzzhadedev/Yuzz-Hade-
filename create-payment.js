...
module.exports = async (req, res) => {
    ...
    if (!amount || !customer_name) {
        return res.status(400).json({ success: false, message: 'Amount dan nama diperlukan' });
    }
    ...
    if (!apiKey || !apiSecret) {
        return res.status(500).json({ success: false, message: 'API Key tidak dikonfigurasi' });
    }
    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                receipt_url: `https://qris.pw/payment/${data.transaction_id}`,
